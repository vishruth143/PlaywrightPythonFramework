# Tutorial: Launching Firefox Browser with Playwright
# -----------------------------------------------------
# This script demonstrates how to launch Mozilla Firefox using Playwright
# and perform a basic search on Google.
#
# Key Concepts:
#   - sync_playwright()               : Context manager that starts the Playwright engine (synchronous API).
#   - playwright.firefox.launch()     : Launches a Firefox browser instance using Playwright's
#                                       bundled Firefox (no GeckoDriver needed — Playwright manages it).
